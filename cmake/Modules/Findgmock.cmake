include( ExternalProject )
include( FindPackageHandleStandardArgs )

set( GMOCK_PREFIX gmock )

set( GMOCK_SOURCE_DIR ${CMAKE_BINARY_DIR}/${GMOCK_PREFIX} )
set( GMOCK_BINARY_DIR ${GMOCK_SOURCE_DIR} )
set( GMOCK_INCLUDE_DIR ${GMOCK_SOURCE_DIR}/include )

set( GTEST_BINARY_DIR ${GMOCK_BINARY_DIR}/gtest )
set( GTEST_SOURCE_DIR ${GMOCK_SOURCE_DIR}/gtest )
set( GTEST_INCLUDE_DIR ${GTEST_SOURCE_DIR}/include )

ExternalProject_Add( gmock
    PREFIX ${GMOCK_PREFIX}
    URL ${CMAKE_SOURCE_DIR}/external/gmock-1.7.0.zip
    URL_MD5 073b984d8798ea1594f5e44d85b20d66
    SOURCE_DIR ${GMOCK_SOURCE_DIR}
    BINARY_DIR ${GMOCK_BINARY_DIR}
    CMAKE_ARGS -DCMAKE_CXX_FLAGS=${CMAKE_CXX_FLAGS}

    UPDATE_COMMAND ""
    INSTALL_COMMAND ""
)

add_library( GMOCK_LIBRARY STATIC IMPORTED GLOBAL )
add_dependencies( GMOCK_LIBRARY gmock )
set_target_properties( GMOCK_LIBRARY PROPERTIES IMPORTED_LOCATION ${GMOCK_BINARY_DIR}/libgmock.a )

add_library( GMOCK_MAIN_LIBRARY STATIC IMPORTED GLOBAL )
add_dependencies( GMOCK_MAIN_LIBRARY gmock )
set_target_properties( GMOCK_MAIN_LIBRARY PROPERTIES IMPORTED_LOCATION ${GMOCK_BINARY_DIR}/libgmock_main.a )

add_library( GTEST_LIBRARY STATIC IMPORTED GLOBAL )
add_dependencies( GTEST_LIBRARY gmock )
set_target_properties( GTEST_LIBRARY PROPERTIES IMPORTED_LOCATION ${GTEST_BINARY_DIR}/libgtest.a )

add_library( GTEST_MAIN_LIBRARY STATIC IMPORTED GLOBAL )
add_dependencies( GTEST_MAIN_LIBRARY gmock )
set_target_properties( GTEST_MAIN_LIBRARY PROPERTIES IMPORTED_LOCATION ${GTEST_BINARY_DIR}/libgtest_main.a )

set( GMOCK_BOTH_LIBRARIES GMOCK_MAIN_LIBRARY GMOCK_LIBRARY )
set( GTEST_BOTH_LIBRARIES GTEST_MAIN_LIBRARY GTEST_LIBRARY )
set( GTEST_ALL_LIBRARIES ${GMOCK_BOTH_LIBRARIES} ${GTEST_BOTH_LIBRARIES} )

find_package_handle_standard_args( GTest
    DEFAULT_MSG
    GMOCK_INCLUDE_DIR
    GTEST_INCLUDE_DIR
)
